...
const Credenza = ({ children, ...props }: RootCredenzaProps) => {
  const isDesktop = useMediaQuery("(min-width: 676px)");
...
const CredenzaContent = ({ className, children, ...props }: CredenzaProps) => {
  ...
  // const ifDialogCLasses = 'max-h-[80vh] rounded overflow-y-scroll overflow-x-hidden no-scrollbar'
  return (
...
const CredenzaBody = ({ className, children, ...props }: CredenzaProps) => {
  return (
    <div className={cn("px-2 md:px-0 h-full", className)} {...props}>
      {children}
    </div>
  );
};
